...
const alertValidation = yup.object().shape({
    ...
    Subject: yup.string().required({ Subject: 'Subject is required' }).max(100, { Subject: 'Subject must be less than 100 characters' }).matches(/^[a-zA-Z]+$/, {message:{ Subject: 'Subject can only have letters' }}),
    
    //Content can only have 250 letters can be letters only
    Content: yup.string().required({ Content: 'Content is required' }).max(250, { Content: 'Content must be less than 250 characters' }).matches(/^[a-zA-Z]+$/, {message:{ Content: 'Content can only have letters' }}),
