...
export const getWebPages = async (count, page, key, val) => {
    try {
        const response = await webpagesService.get(`http://localhost:4000/api/pros/Access/AccessLists/${count}/${page}?key=${key}&val=${val}`);
...
export const getWebPagesCount = async (key, val) => {
    try {
        const response = await webpagesService.get(`http://localhost:4000/api/pros/Access/AccessList/count?key=${key}&val=${val}`);
        return response.data;
    } catch (error) {
        throw error;
    }
...
export const getPagesByDatetime = async (count, page, start, end, key, val) => {
    ...
    try {
        const response = await webpagesService.get(`http://localhost:4000/api/pros/web/webpages/datetime/${count}/${page}?start=${start}&end=${end}&key=${key}&val=${val}`);
...
export const getPagesByDatetimeCount = async (start, end, key, val) => {
    try {
        const response = await webpagesService.get(`http://localhost:4000/api/pros/Access/AccessList/datetime/count?start=${start}&end=${end}&key=${key}&val=${val}`);
        console.log(response.data);
        return response.data;
    } catch (error) {
        throw error;
